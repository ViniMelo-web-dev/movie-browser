...
const MoviePage = () => {
  ...
  const {title, release_date, vote_average, vote_count,
    poster_path, genres, revenue, tagline, production_companies, 
    bugdet, spoken_languages, status, 
    production_countries, overview, runtime} = movie;
    ...
    <div className='bg-dark-100 bright-shadow rounded-[8px] p-7 max-w-full'>
      <section className='header flex justify-between'>
        ...
        <div className='bg-[#221F3D] rounded-[4px] p-2'>
          <div className='flex items-center'>
            ...
            <p className='text-gray-100'><span className='text-white font-bold'>{vote_average ? vote_average : 'N/A'}</span>/10<span className='mx-1.5'>({vote_count ? vote_count : 'N/A'})</span></p>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className='mt-10 flex gap-7 items-center'>
          <img className='w-[18.87rem] h-[27.5] rounded-[10px]' src={`https://image.tmdb.org/t/p/w500/${poster_path}`}></img>
          <div className='w-full'>
            {movieTrailer ? (
              <iframe className='rounded-[10px] w-full'
                width="560"
                height="440"
